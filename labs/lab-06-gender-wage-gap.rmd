...
This lab draws on literature on the "gender wage gap". The language around *gender* is imprecise and outdated. We know that gender and biological sex are different constructs, and that neither are perfectly binary. 

For the purpose of this work we acknowledge the limitations of both the constructs and the language around gender vs sex and treating male/female as binary. This research should be considered an imperfect jumping off point that can be further refined when better data is available. 

For example, [Apple Magic Sauce](https://applymagicsauce.com/demo) can predict your gender continuum from your tweets or Facebook posts. Gender is measured on a scale of 1 (completely masculine) to 100 (completely feminine) with most people falling somewhere inbetween and a score of 50 representing an androgynous gender identity. Public text like Twitter data could be used to assign a gender score to each CEO and replicate this study using a fluid instead of binary version of gender. These innovations are welcome! 
...
Note that this study differs from other studies where the treatment (male in this case) and control (female) can be assigned. In this study gender is pre-determined and static, so we have no control over assignment of the treatment group (we could not convince CEOs to undergo gender reassignment for the study). 

We often use a perfect experiment (RCT) as an intellectual jumping-off point to think about research design. There are lots of interesting cases where we want to examine the impact of a non-assignable type of treatment. In these cases we need to be very clever about research design to make sure we are estimating the effects of the group and not differences in how group members behave. 

For example, researchers have used Uber data to study the wage gap because pay is assigned by an algorithm that does not know the gender of individual drivers and thus can by design not be implicitly or explicitly biased in paying male or female drivers differently. Observed differences in hourly pay result from behavior or performance only. One interesting finding, for example - men drive faster than women, and thus make slightly more. 

Other examples include studies that send identical resumes to job ads but vary the name on the resume, using names that are clearly black or clearly white. Since all of the qualifications (schools, degrees, age, past performance at previous jobs) are identical except the name, one can claim that differences in response rates are entirely a function of race and not other considerations. 

The thought experiment here would be two identical candidates for a CEO job, one a man, one a woman, but otherwise qualifications are the same. Would the typical CEO salary offer for the two candidates differ because of gender? 
...
* Each estimator will produce very different answers. 
* Your job is to explain what each estimator represents, then choose which estimator provides the most defensible counterfactual for measuring the discrimination piece of the wage gap. 
...
Which estimator best captures discrimination?  
...
**It is all too common for unskilled analysts to use models they don't fully understand, then simply to look for the coefficient associated with the policy variable without understanding the counterfactual encoded in the model.** 
...
Learn to take a step back, think about how you would ideally construct a counterfactual, then ask whether your current model is generating the correct estimate of program impact. 

Examples like this will help you appreciate how powerful regression can be when used with knowledge and intent. 
...
This lab focuses on the conceptual interpretation of study design, as opposed to the interpretation of regression tables. 

Specifically, you are invited to explore the operationalization of the concept of the wage gap. It can be decomposed into different meaningful components. 

* Certain industries are "gendered", meaning that workers in the industry have historically been predominantly one gender. Industry pay reflects values of a society or historical path-dependencies related to that profession (teaching in primary schools in the past was quasi-voluntary work that did not require credentials), but not discrimination at the organizational level. 
* Women make different career choices than men, such as selecting jobs with more flexibility and less pay. These are choices that reflect individual preferences and do not constitute discrimination, per se. The origins of preferences is a separate but consequential topic. 
* Discriminatory pay practices are specifically instances where two people perform the same work and receive different pay, and differences are systematic by some protected class status. "Same work" is typically operationalized by job title within an organization. 

In this case we are asked to estimate the level of **wage discrimination** in the sector. 
...
Part of the sorting is due to the fact that men are *perceived to be* better prepared for leadership positions in large organizations (this assumption is not supported by data but persists). This might happen at the stage of identifying talent in early stages of careers and providing mentoring and training that prepares people for leadership positions. Part of it occurs during the interview process, when genderized perceptions of leadership can disadvantage candidates. 

The third component occurs after someone is hired for a position. It is at this point that pay structure is set for people in leadership positions, often reflecting past experience, opportunity costs for the candidate, and the perceived value they will bring to the organization. 

This stage is where pay discrimination would occur - when the committees that determine the salary offers systematically pay one class (women) lower than their equally-qualified male counterparts. 

This typically occurs as a result of things like implicit bias or different negotiation tactics, not any malicious intent. Intent is not necessary for discrimination to occur. 

Different industries or organizations will exhibit different mixes of each of these three components:
...
Or more precisely, this case is interested in **pay discrimination**. The component of the gap that results from **discriminatory or unfair hiring** practices would require a different study to separate voluntary sorting from gatekeeping in firms. 
...
For convenience we can refer to one gender as the "treatment" and one as the "control". I have picked male to represent the treatment group here only because we know salaries will be higher and we typically expect the treatment to increase performance or resources. So it is cognitively consistent but otherwise arbitrary. 

**Thought Experiments:**

When tackling a hard research design problem it is often useful to start with a thought simulation - what would the ideal experiment look like in this study? 

If we want to isolate the effect of gender on CEO pay, what is the counterfactual? 
